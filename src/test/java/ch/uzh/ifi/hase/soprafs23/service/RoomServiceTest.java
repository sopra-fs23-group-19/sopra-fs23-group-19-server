...
public class RoomServiceTest {
    ...
    public void availableRooms_success() {
        ...
        Mockito.when(userRepository.findByRoomId(Mockito.anyLong())).thenReturn(users);
    ...
    public void getAllPlayersIds_success() {
        ...
        room.setOwnerId(1L);
        room.setStatus(RoomStatus.WAITING);
        Mockito.when(roomRepository.findByid(Mockito.anyLong())).thenReturn(room);
        User user = new User();
        user.setPassword("Firstname Lastname");
        user.setUsername("firstname@lastname");
        ...
        List<Long> allPlayerIds = roomService.getAllPlayersIds(room.getId());

        assertEquals(1L, allPlayerIds.get(0));
    }
